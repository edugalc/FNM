...
export default function UploadPortada({ ebookId }: { ebookId: number }) {
  ...
  const upload = async () => {
    ...
    await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/ebooks/${ebookId}/upload-portada`,
      {
        method: "POST",
        headers: {
  ...
  return (
    <div className="p-5 bg-white shadow rounded-xl space-y-3">
      <h3 className="font-semibold">Subir Portada</h3>
